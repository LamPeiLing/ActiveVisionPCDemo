...
from sympy.solvers.solveset import ImageSet
import tqdm
...
class CourtDetector:
  ...
  def infer_model(self, frames):
      ...
      kps_res = []
      for num_frame, image in enumerate(frames):

          if num_frame == 0:
            lines = self.detect(image)
          else:
            lines = self.track_court(image)

          kps_res.append(lines)

      return self.game_warp_matrix, kps_res
  ...
  def track_court(self, frame):
        ...
        # Check for None values in the intersections (collinear case)
        if i1 is None or i2 is None or i3 is None or i4 is None:
            print("Collinear lines detected, cannot find valid intersection points.")
            # Handle collinear case: reset or re-detect the court
            return None
...
def line_intersection(line1, line2):
    ...
    # if intersection:  # Ensure the intersection exists
        # If intersection[0] is a tuple or point, return it directly
        return intersection[0].coordinates  # Adjust if needed based on the structure of intersection[0]
    elif isinstance(intersection[0], Line2D):
        # If it's a Line2D, handle the case where lines are collinear
        print("The lines are collinear.")
        # You can return None or handle this case as needed
        return None
    else:
        raise ValueError("Lines do not intersect or intersection cannot be found.")
